...
- **文件移动**: 移动 `src/` 目录中的文件时，必须同步移动并调整测试目录中对应的测试文件，保持模块与测试路径一致。
...
- **异常处理**: 抛出异常时优先使用模块内部异常基类继承的专用异常；没有合适的异常时创建并使用模块专属异常，避免直接抛出通用异常。
- **核心文件**:
...
- **README 更新**:
  ...
  - Git历史查询注意事项: 避免使用`--follow`，`git log --follow` 仅支持单个文件；多个 README 需分别查询。
