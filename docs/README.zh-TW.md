...
-   **`tomorrow`**: 核心智能體模組。代號取自遊戲《死亡擱淺 2：冥灘之上》（Death Stranding 2: On the Beach）中的角色**明天**（由艾丽·范宁饰演）。在劇情中，她是主角山姆布里吉斯（Sam Bridges）的女兒，也被揭露為前作中的**大樓**(BB-28)。
...
-   **成為**: 必須正在運行且可存取。
-   **LLM 模型**: 預設使用 Ollama 的`qwen3.5:9b`。您可以使用以下命令取得：
    ```bash
...
該項目使用**懸垂設定** 进行配置管理。设置分别定义在 `src/tomorrow/settings.py`（明天）和`src/rainy/settings.py`(Rainy) 中，可以透過環境變數或`.env`文件進行覆蓋。環境變數優先權最高，Tomorrow 使用`TOMORROW_`前綴，Rainy 使用`RAINY_`前綴。
...
| `TOMORROW_MODEL`      | 模型配置，支援 OLLAMA、HUGGINGFACE 和 ANTHROPIC | `{"type": ModelType.OLLAMA, ...}`  |
...
模型配置透過`TOMORROW_MODEL`傳入。使用 Anthropic 時，將模型類型設定為`anthropic`，並提供模型名稱和 API 金鑰，例如：

```bash
export TOMORROW_MODEL='{"type":"anthropic","anthropic":{"model":"claude-sonnet-5","api_key":"your-anthropic-api-key"}}'
```

使用 Ollama 或 HuggingFace 時，可分別配置`ollama`或`huggingface`對象；具體欄位和預設值請參閱`src/tomorrow/settings.py`。
...
项目使用 `pytest` 进行测试，并要求 **100%**的測試覆蓋率。
