...
このプロジェクトは、`deepagents`フレームワークはユーザー入力を分析して渡します`rainy` 模块对外提供同步（`/api/chat`）そして**ストリーミング (`/api/chat/stream`）**APIインターフェース。
...
-   **コードの実行**:[langchain-quickjs](https://github.com/langchain-ai/langchainjs) 提供的 QuickJS 中间件
...
サブエージェント構成が渡されました`TOMORROW_SUBAGENTS`渡される各サブエージェントには少なくとも次のものが必要です`name`、`description`そして`system_prompt`フィールドに指定することもできます`model`そして`skills`，例如：
...
-   `src/fragile/`: コマンド ライン クライアント パッケージ ディレクトリ。
    -   `app.py`: インタラクティブなセッションを提供し、スレッド UUID によるセッションの再開をサポートします。
