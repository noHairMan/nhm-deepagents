...
`fragile`CLI エントリと`purge`サブコマンドはすべて非同期コマンド関数を使用して、すでに実行中のイベント ループでのネストされた呼び出しを回避します。`asyncio.run()`。永続化されたセッション レコードをクリーンアップします。
...
-   `src/fragile/`: コマンド ライン クライアント パッケージ ディレクトリ。
    -   `app.py`: 使用`asyncclick`非同期を定義する`fragile`命令行入口和`purge`子命令。
