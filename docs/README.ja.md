...
-   **LLM 模型**: デフォルトで使用されるモデルは`qwen3.5:9b`。次のコマンドを使用して取得できます。
    ```bash
...
| `TOMORROW_BACKEND`         | バックエンド構成、サポート`FILESYSTEM`そして`DAYTONA` | `{"type": BackendType.FILESYSTEM}` |
...
-   `src/tomorrow/`: コア エージェント パッケージ ディレクトリ。
    -   `core/agent.py`: ディープ エージェントとその命令を定義します。`AgentManager`ライフサイクル管理、ファイルシステムおよびデイトナ サンドボックス バックエンドのサポート (ローカル モードのみ)。
    -   `core/checkpoints/`: チェックポイントの実装 (メモリ、SQLite など)。
    -   `models/constants/backend.py`: バックエンドタイプの定数を定義します。
