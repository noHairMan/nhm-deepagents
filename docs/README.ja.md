...
-   **交互式 CLI**:`fragile`支持`/new`新しいセッションを作成し、`/history`永続化された履歴セッションを参照して切り替えます。`/account`外部モデルアカウントを設定し、`/model`モデルを選択し、`/quit`終了、セッションの再開、入力履歴、スラッシュ コマンドの補完、複数行の編集、モデルの概要、ツールの呼び出し、コマンドの結果、最終的な回答を示すタイムラインが追加されています。
...
-   `Assistant`: 最終的な答えは、モデルによって返されるフラグメントとして引き続き継続的にストリーミングされます。
