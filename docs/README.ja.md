...
-   **`fragile`**: Tomorrow エージェントに直接質問したり、対話型セッションを開始したりするための、Typer ベースのコマンド ライン クライアント。名前は同作の登場人物から。**壊れやすい**。 Fragile は Fragile Express の創設者であり配達員です。彼は時の雨にさらされて急速に老化しましたが、危険な環境で常に重要な物資を他の人に届けてきました。つなぐ、届けるという使命を今も主張する「もろさ」のイメージが、このクライアント名の背景となっている。
...
このプロジェクトでは、**ピダンティックな設定**構成管理を実行します。設定はそれぞれ次のように定義されています。`src/tomorrow/settings.py`（明日）、`src/rainy/settings.py`(雨) そして`src/fragile/settings.py` (Fragile) 中，可以通过环境变量或 `.env`ファイルが上書きされます。環境変数は最も高い優先順位を持ち、3 つのモジュールによってそれぞれ使用されます。`TOMORROW_`、`RAINY_`そして`FRAGILE_`接頭語;渡すこともできます`TOMORROW_ENV_FILE`、`RAINY_ENV_FILE`または`FRAGILE_ENV_FILE`設定ファイルのパスを指定します。
...
-   `src/fragile/`: コマンド ライン クライアント パッケージ ディレクトリ。
    ...
    -   `commands/interactive/`: インタラクティブなセッション実装。セッション回復、新しいセッション、コマンド補完、複数行入力をサポートします。
        ...
        -   `commands/`: インタラクティブなスラッシュ コマンドの実装とコマンド レジストリ。
            -   `base.py`: 基本的なインターフェイスと対話型コマンドの処理結果を定義します。
            -   `history.py`: 永続化された履歴セッションをクエリして選択します。
            -   `new.py`: 新しいセッションを作成します。
            -   `quit.py`: インタラクティブセッションを終了します。
        -   `display.py`：管理端末の表示。
        -   `input.py`: 入力履歴、コマンド補完、複数行編集を管理します。
        -   `session.py`: インタラクティブなセッションのフローを管理します。
    -   `conf/config.py`: 遅延ロードされた CLI グローバル設定を提供します。
    -   `models/session.py`: インタラクティブなセッション状態モデルを定義します。
    -   `models/constants/command.py`: コマンド処理結果の定数を定義します。
    -   `settings.py`: CLI モジュールのデフォルト設定。
