...
プロジェクトには現在、使用できる一般的なスマート アシスタント エージェントが含まれています`deepagents`このフレームワークはユーザー入力を分析し、次の方法で提案を提供します。`rainy` 模块对外提供同步（`/api/chat`) およびストリーミング (`/api/chat/stream`) API インターフェース。
...
環境変数にはデフォルトで接頭辞が付けられます`TOMORROW_`(コアモジュール) または`RAINY_` (API 模块) 开头。
